...
class WepayControllerOauth extends WepayController {
 ...
 var $error = NULL;
  ...
  var $accountCreate = NULL;
    ...
    $this->accountCreate = $wepay->request('account/create/', array(
        'name'          => $this->user->username,
        'description'   => Wepay::getInstance()->get('app_description', 'Funding Application')
    ));
	...
	function display($cachable = false, $urlparams = false) {
		...
		DSCTable::addIncludePath( JPATH_ADMINISTRATOR.'/components/com_wepay/tables' );
		$table = DSCTable::getInstance( 'Accounts', 'WepayModel' );
		$table->load();	
